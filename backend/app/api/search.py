...
def _keyword_ranking(
    ...
    ids_staged=False, max_agreement=None, cluster=None, album=None,
    ...
    where, params = build_filters(split, tag, vlm_tag, attr, axes, ids, ids_staged,
                                  max_agreement, cluster=cluster, album=album)
...
def run_search(
    ...
    # Keyword arguments past the first four: this call is long enough that a
    # bare `None` in a positional slot says nothing about which filter it is,
    # and two adjacent Optional[int]s (cluster, album) made a silent swap one
    # edit away.
    allowed = filtered_id_set(conn, split, tag, vlm_tag, attr=attr, axes=axes,
                              ids=ids, ids_staged=ids_staged,
...
def search(
    ...
    cluster: Optional[int] = Query(
        None, ge=-MAX_SQLITE_INT - 1, le=MAX_SQLITE_INT,
        description="Restrict candidates to one k-means cluster"),
...
def search_post(body: SearchRequest, conn: sqlite3.Connection = Depends(get_conn)):
    ...
    return run_search(conn, body.q, mode=body.mode, top_k=body.top_k,
                      ...
                      cluster=body.cluster, album=body.album)
