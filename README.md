...
- **Curation workflow, closing both ways** — the point of a search tool over a dataset is composing a training set, so a slice has to be able to leave and come back:
  ...
  - Tag samples manually, in bulk by lassoing a region of the map, by hand-picking in the gallery (**Select** mode — an "album" is a tag made by selection, so it is revisitable, shareable and exportable like any slice), or via the assistant; filter by tag.
